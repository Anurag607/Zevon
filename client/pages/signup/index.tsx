...
type signupTemplate = { cnfrmpass: string, email: string, name: string, password: string, user_type: string }
        ...
        if(signupdet.name.length == 0) {
            ...
            .then(resMessage => {
                if(status === 201) {
                    ...
                    styling.warning.current!.style.display = 'block'
                    styling.warning.current!.innerHTML = (resMessage.message === 'User already exists') ? `${resMessage.message}` : `Failed To Create Your Account! Try Again in a Few Minutes.`
                    styling.username.current!.style.border = '0.05rem solid red'
                    styling.heading.current!.style.marginBottom = '0.5rem'
                }
            })
