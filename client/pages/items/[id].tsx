...
const Page = ({
  ...
  const itemDetails = React.useRef<productType>(
    JSON.parse(Cookie.get("currentItem") || "{}")
  );
  ...
  const [cart, setCart] = React.useState<productType[]>(
    JSON.parse(Cookie.get("cart") ? Cookie.get("cart") : "[]")
  );
  ...
  const select = () => {
    ...
    if (typeof colorIndex === "undefined" || typeof colorIndex === "string")
      colorIndex = 0;
    const color: NodeListOf<HTMLSpanElement> =
      document.querySelectorAll(".colors");
    const halo: NodeListOf<HTMLSpanElement> =
      document.querySelectorAll(".colorHalo");
    color[colorIndex].style.transform = "scale(1.1)";
    halo[colorIndex].style.width = `${
      color[colorIndex].offsetWidth * 0.085
    }rem`;
    halo[colorIndex].style.height = `${
      color[colorIndex].offsetHeight * 0.085
    }rem`;

    if (typeof sizeIndex !== "undefined" && typeof sizeIndex === "number") {
      const sizes: NodeListOf<HTMLElement> =
        document.querySelectorAll(".sizes");
      if (typeof itemDetails.current !== "string")
        itemDetails.current.size = sizes[sizeIndex].textContent;
  ...
  React.useEffect(() => {
    ...
    select();
    if (typeof itemDetails.current !== "string")
      itemDetails.current.quantity = 1;
  }, []);

  const updateCart = (props: React.MutableRefObject<productType>) => {
    const itemExist = cart.find((item) => {
      return (
        item.product_id === props.current.product_id &&
        item.color === props.current.color &&
        item.size === props.current.size
      );
    });
    if (!itemExist) {
      ...
      tempCart.map((el: productType, i: number) => {
        if (
          el.product_id === props.current.product_id &&
          el.color === props.current.color &&
          el.size === props.current.size
        ) {
          tempCart[i] = {
            ...el,
            ...
            quantity:
              el.quantity + props.current.quantity > 2
                ? el.quantity
                : el.quantity + props.current.quantity,
          };
        }
      });
      Cookie.set("cart", JSON.stringify(tempCart));
  ...
  const HandleColorClick = (
    ...
    Cookie.set(
      "selectedParams",
      JSON.stringify({
        item_id: itemDetails.current.product_id,
        color: selectedParams.color.current,
        size: selectedParams.size.current,
      }),
      { path: "" }
    );
    deSelectColor();
  ...
  const SizeRen = (props: { size: string; index: number }) => {
    return (
      <li
        onClick={(event) => HandleSizeClick(event, props.index)}
        className="sizes"
      >
        {props.size}
      </li>
  ...
  const Colorgen = (props: { color: string; index: number }) => {
    return (
      <div
        className={`${styles.colorCont} colorCont`}
        onClick={(event) => HandleColorClick(event, props.index)}
      >
        <span
          className={`${styles.Colors} colors`}
          style={{
            backgroundColor: `${
              props.color === "others"
                ? "blue"
        ...
        />
        <span
          className={`${styles.ColorHalo} colorHalo`}
          style={{
  ...
  const ColorGen = () => {
    let colors = [
      `${product_details.color ? product_details.color : "#4169e1"}`,
      "#ffd300",
      "#37474f",
      "#f8bbd0",
      "#32cd32",
    ...
    <div className={styles.itemWrapper}>
      ...
      <div className={styles.itemContent}>
        ...
        <section className={`${styles.itemDetails} itemDetails`}>
          ...
          <QtyGen
            qty={product_details.quantity ? product_details.quantity : 5}
          />
        ...
        <div
          ...
          style={{ display: `${warning.flag ? "flex" : "none"}` }}
        >
          <div>
            <span
