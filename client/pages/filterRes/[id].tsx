...
const Page = ( {products} : {products: filterParamsType[] | string[]} ) =>  {
    ...
    const Fetcher = () => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_LOCALHOST_SERVER}/api/product/filtered`, {
            ...
            body: JSON.stringify(JSON.parse(Cookie.get('filterParams') ? Cookie.get('filterParams') : `{}`))
        })
        .then(response => {
    ...
    const ProductList = () => {
        ...
        return (
            <section className={`${styles.resList} resList`}>
                {result.map((el: filterParamsType | string,i: React.Key) => {
                    return <Cards key={i} type='shopping' details={el} image={img_sources[i]}/>
                })}
            </section>
        )
    }
    ...
    const FilterBar = () => {
        ...
        const categories = ["Shirts & Tops", "Pants & Lowers", "Jackets", "Accessories", "Shoes & Footwear"]
                ...
                row.push(
                    <div className={styles.filterColorRow}>
                        <span key={index} data-color={hex[index]} className='filterColor'>
                            <input type="checkbox" id={`${colors[index]}`} name="color" value={`${colors[index]}`} />
                            <label htmlFor={`${colors[index]}`}>{`${colors[index]}`}</label>
                        </span>
                        <span key={index+1} data-color={hex[index+1]} className='filterColor'>
                            <input type="checkbox" id={`${colors[index+1]}`} name="color" value={`${colors[index+1]}`} />
                            <label htmlFor={`${colors[index+1]}`}>{`${colors[index+1]}`}</label>
                        </span>
                    </div>
                )
                ++index
            ...
            return (
                <div className={styles.filterCategory}>
                    ...
                    {categories.map((el,i) => {
                        return (
                            <span key = {i}>
                                <input type="checkbox" id={`${type[i]}`} name="category" value={`${type[i]}`} />
        ...
        return (
            <section className={`${styles.filterOptions} filterOptions`}>
                <form className={`${styles.filterForm} filterForm`} onSubmit={HandleSubmit}>
                    ...
                    <div className={styles.filterCost}>
                        ...
                        <span>
                            <input type="checkbox" id="average" name="cost" value="500,1000" />
                            <label htmlFor="average">₹500-₹1000</label>
                        </span>
                        <span>
                            <input type="checkbox" id="intermediate" name="cost" value="1000,1500" />
                        ...
                        <span>
                            <input type="checkbox" id="expensive" name="cost" value="2000,2500" />
    ...
    return (
        <div className={styles.filterResWrapper}>
            ...
            <div className={styles.resContent}>
                <section>
                    <div className={`${styles.filterToggle} filterToggle`} data-toggle='close' onClick={(event) => {
                        const target = event.currentTarget
                        const filter:HTMLElement = document.querySelector('.filterOptions')
...
export async function getServerSideProps( {params, req, res} : {params: {id: string}, req: NextApiRequest, res: NextApiResponse} ) {
