...
const Page = ({userDetails} : {userDetails: string}) => {
    ...
    const [addr, SetAddr] = React.useState({
        ...
        city: (typeof auth.current.address.city !== 'undefined' && auth.current.address.city !== null) ? auth.current.address.city : '',
        pincode: (typeof auth.current.address.pincode !== 'undefined' && auth.current.address.pincode !== null) ? `${auth.current.address.pincode}` : '',
        country: (typeof auth.current.address.country !== 'undefined' && auth.current.address.country !== null) ? auth.current.address.country : ''
    })
    ...
    const addrFetcher = () => {
        ...
        let address = {
            user_id: auth.current.user_id,
            addr_1: (addr.address.length > 128) ? addr.address.slice(0,127) : addr.address,
            addr_2: (addr.address.length > 128) ? addr.address.slice(128,addr.address.length-1) : '',
            city: addr.city,
        ...
        .then(resMessage => {
            if(status !== 201) {
                ...
                auth.current.address = {
                    ...
                    user_id: auth.current.user_id,
                    address_line1: (addr.address.length > 128) ? addr.address.slice(0,127) : addr.address,
                    address_line2: (addr.address.length > 128) ? addr.address.slice(128,addr.address.length-1) : '',
                    city: addr.city,
    ...
    const cardValidation = () => {
        ...
        const cardnumLabel:HTMLLabelElement = document.querySelector("#cardnumLabel")
        const masterCardrgx = /^5[1-5][0-9]{14}|^(222[1-9]|22[3-9]\\d|2[3-6]\\d{2}|27[0-1]\\d|2720)[0-9]{12}$/
                    ...
                    <div className={styles.prsnlblockWrapper}>
                        ...
                        <div className={styles.prsnlForm}>
                            ...
                            <div className={styles.others}>
                                ...
                                <div className={styles.inputField}>
                                    <input type="tel" name="phnum" id="phnum" value={phNum} onChange={HandleChange} required />
                    ...
                    <div className={styles.shippingblockWrapper}>
                        ...
                        <div className={styles.shippingForm}>
                            <div className={styles.address}>
                                <div className={styles.inputField}>
                                    <input type='text' name='addr' id='addr' value={addr.address} onChange={HandleChange} required />
                                    <label htmlFor={'addr'}>address</label>
                                </div>
                            </div>
                            <div className={styles.others}>
                                <div className={styles.inputField}>
                                    <input type="text" name="pin" id="pin" value={addr.pincode} onChange={HandleChange} required />
                                    <label htmlFor={"pin"}>{"pin code"}</label>
                                </div>
                                <div className={styles.inputField}>
                                    <input type="text" name="city" id="city" value={addr.city} onChange={HandleChange} required />
                                    <label htmlFor={"city"}>{"city"}</label>
                                </div>
                                <div className={styles.inputField}>
                                    <input type="text" name="country" id="country" value={addr.country} onChange={HandleChange} required />
