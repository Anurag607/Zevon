...
const Page = ({cookieCart, userDetails} : {cookieCart: string, userDetails: string}) => {
    ...
    const sendMail = () => {
        ...
        fetch(`${process.env.NEXT_PUBLIC_PRODUCTION_SERVER}/api/email/confirmation`, {
            ...
            body: JSON.stringify({user: userDetails, content: document.querySelector('.item').innerHTML})
        }).then(response => {
    ...
    React.useEffect(() => {
        ...
        setUserAddr(`${(auth.current.address.address_line1 !== null) ? auth.current.address.address_line1 : ''} ${(auth.current.address.address_line2 !== null) ? auth.current.address.address_line2 : ''} ${(auth.current.address.city !== null) ? ', ' + auth.current.address.city : ''} ${(auth.current.address.country !== null ) ? ', ' + auth.current.address.country : ''} ${(auth.current.address.pincode !== null) ? '- ' + auth.current.address.pincode : ''}`)
    }, [])
    ...
    return( 
        <div className={styles.orderDetailsWrapper}>
            ...
            <NavBar />
            <div className="item">
                <div style={{width: '100vw', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '9.375rem'}}>
